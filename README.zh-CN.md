...
- **长期记忆** — 跨会话持久化项目知识、用户偏好和技术方案，存储在 `.neonity/memory.md`。
...
│   └── context-manager.ts  Context Window 管理和压缩
├── memory/
│   ├── memory.ts         长期记忆系统（持久化到 .neonity/memory.md）
│   └── memory-tool.ts   Agent 使用的记忆工具
├── provider/
│   ├── provider.ts       统一导出
